...
- **No parent-relative imports.** Use the workspace package name
  ...
  bundler plugins. Sibling imports (`./xxx`) are fine — they don't
...
- **Named imports, not namespace imports.** Use
  `import { foo, bar } from "./module"` instead of
  `import * as mod from "./module"`. Drop the `.ts` extension from
  sibling imports (`from "./helpers"`, not `from "./helpers.ts"`).
