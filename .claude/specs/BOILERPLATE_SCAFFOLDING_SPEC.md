...
12. **Pinned TypeScript to exact `5.9.3`** (no caret) across all `package.json` files. pnpm's auto-update behavior bumped TS to 6.0.3, which deprecates `baseUrl` (a tsconfig path-resolution change) and would have required a tsconfig migration. Holding on 5.9 until the React/Vite ecosystem catches up.
