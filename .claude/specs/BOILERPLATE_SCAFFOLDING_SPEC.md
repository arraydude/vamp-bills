...
7. **Enabled React Compiler + upgraded to Vite 8.** Added `babel-plugin-react-compiler` so React 19's auto-memo handles `useMemo`/`useCallback`/`React.memo` for us. Three integration notes with Vite 8:
   - **`@vitejs/plugin-react` v6 dropped its `babel` option** (refresh transform moved to Oxc). React Compiler now runs as a separate Babel pass.
   - **`@rolldown/plugin-babel`** (the Rolldown-native integration) is the right plugin per Vite 8's migration guide — *not* the legacy `vite-plugin-babel`. The native version is ~40% faster (build: 524ms → 311ms) and avoids the `optimizeDeps.esbuildOptions` deprecation warning.
   ...
   Final wiring: `[react(), babel({ presets: [reactCompilerPreset()] }), tailwindcss()]` where the preset returns `{ presets: ['@babel/preset-typescript'], plugins: ['babel-plugin-react-compiler'] }` and a `rolldown.filter.id: /\.[jt]sx?$/` for efficiency. Bundle: 234 → 231 KB JS, CSS: 22 → 23 KB.
...
9. **Added GitHub Actions CI** (`.github/workflows/ci.yml`): runs `biome ci`, `pnpm typecheck`, and `pnpm build` on every PR to `develop`/`main` and every push to those branches. Uses pnpm 10.9.0 and Node 20 with concurrency cancel-in-progress.
